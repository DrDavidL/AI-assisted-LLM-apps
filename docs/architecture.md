...
| LLM | OpenAI GPT-4o | Structured output generation of medical cases |
...
3. **LLM Service** calls `openai.AsyncOpenAI.beta.chat.completions.parse()` with `response_format=MedicalCase` — the API returns structured JSON matching our Pydantic schema
